...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_INTEGER(N_SA1_sydney);
  DATA_STRUCT(spde_sydney,spde_t); // INLA SPDE object (components of precision matrix)
  DATA_SPARSE_MATRIX(A_sydney); // INLA SPDE projection matrix: mesh to pixels [dim: NMB x nMesh]
  ...
  PARAMETER_VECTOR(field_sydney); // [dim: nMesh_sydney]
  ...
  SparseMatrix<Type> Q_sydney = Q_spde(spde_sydney,kappa);
  nll += SCALE(GMRF(Q_sydney),sd)(field_sydney);
  ...
  vector<Type> baseline_field_sydney(N_SA1_sydney);
  baseline_field_sydney = A_sydney*field_sydney;
  
  matrix<Type> pollution_expanded(N_SA1_sydney,N_pollution);
  vector<Type> buffer(N_SA1_sydney);
  ...
  vector<Type> pollution_one(N_SA1_sydney);
  vector<Type> pollution_two(N_SA1_sydney);
  vector<Type> pollution_four(N_SA1_sydney);
  ...
  vector<Type> pollution(N_SA1_sydney);
  ...
  vector<Type> dry_cold(N_SA1_sydney);
  dry_cold = dry_cold_two.array();
  
  matrix<Type> humid_hot_expanded(N_SA1_sydney,N_humid_hot);
  ...
  vector<Type> humid_hot_two(N_SA1_sydney);
  ...
  vector<Type> humid_hot(N_SA1_sydney);
  humid_hot = humid_hot_two.array();
  
  matrix<Type> vegetation_expanded(N_SA1_sydney,N_vegetation);
  ...
  vector<Type> vegetation_two(N_SA1_sydney);
  ...
  vector<Type> vegetation(N_SA1_sydney);
  vegetation = vegetation_two.array();
  
  vector<Type> ses_effects(N_SA1_sydney);
  for (int i=0; i<N_SA1_sydney; i++) {ses_effects[i] = ses_slopes[ses[i]];}
  
  vector<Type> predicted_surface_asthma_sydney(N_SA1_sydney);
  predicted_surface_asthma_sydney = intercept_sydney  + baseline_field_sydney.array();
  
  vector<Type> predicted_surface_asthma(N_SA1_sydney);
  predicted_surface_asthma = predicted_surface_asthma_sydney.array() + pollution.array() + dry_cold.array() + humid_hot.array() + vegetation.array() + ses_effects.array() + exp(log_lstdiff_slopes)*lstdiff.array() + exp(log_interaction_pollution_hot)*humid_hot.array()*pollution.array() + exp(log_interaction_pollution_cold)*dry_cold.array()*pollution.array() + exp(log_interaction_vegetation_hot)*humid_hot.array()*vegetation.array() + exp(log_interaction_vegetation_cold)*dry_cold.array()*vegetation.array();
  predicted_surface_asthma = invlogit(predicted_surface_asthma).array();
  
  // Likelihood
  
  for (int i=0; i<N_SA1_sydney; i++) {
  ...
  REPORT(vegetation);
  REPORT(ses_effects);
  vector<Type> lstdiff_effects(N_SA1_sydney);
